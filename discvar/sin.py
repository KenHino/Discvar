...
class Sine(DVRPrimitivesMixin):
    ...
    def _transformed_var(self, x) -> np.ndarray:
    ...
    def get_pos_rep_matrix(self) -> np.ndarray:
        ...
        .. math::
           Q_{j k}=\left\langle\varphi_{j}|\hat{z}| \
            ...
            =\frac{1}{2}\left(\delta_{j, k+1}+\delta_{j, k-1}\right)
        ...
        if self.doAnalytical:
            ...
            logger.warn(
                "Sine DVR position operator "
                + "is somehow different from straightforward integral."
                + "See https://doi.org/10.1016/S0370-1573(99)00047-2"
                + "and Set doAnalytical=True"
            )
    ...
    def get_1st_derivative_matrix_fbr(self):
        ...
        .. math::
           ...
           \varphi_{k}\right\rangle
           ={\rm mod} (j-k, 2) \frac{4}{L} \frac{j k}{j^{2}-k^{2}},\
           \quad j \neq k
    ...
    def get_2nd_derivative_matrix_dvr(self):
        ...
        .. math ::
           ...
           =-\left(\frac{\pi}{\Delta x}\right)^{2}
           \left\{\begin{array}{l}
           -\frac{1}{3}+\frac{1}{6(N+1)^{2}}
           -\frac{1}{2(N+1)^{2} \sin ^{2}\left(\frac{\alpha \pi}{N+1}\right)},
           \quad \alpha=
           \beta \\
           \frac{2(-1)^{\alpha-\beta}}{(N+1)^{2}}
           \frac{\sin \left(\frac{\alpha \pi}{N+1}\right)
           \sin \left(\frac{\beta \pi}{N+1}\right)}
           {\left(\cos \left(\frac{\alpha \pi}{N+1}\right)
           -\cos \left(\frac{\beta \pi}{N+1}\right)\right)^{2}},
           \quad \alpha \neq \beta
           \end{array} \right.
        ...
        if self.doAnalytical:
            if not hasattr(self, "second_derivative_matrix_dvr"):
                ...
                for alpha, beta in itertools.product(
                    ...
                    bp_N = beta * pi / _N
                    if alpha == beta:
                        # Beck's 2000 thesis analytical
                        # form is wrong. (sign of first term)
                        val = (
                            +1 / 3
                            + 1 / (6 * (_N**2))
                            - 1 / (2 * ((_N * sin(ap_N)) ** 2))
    ...
    def diagnalize_pos_rep_matrix(self):
        ...
        .. math::
           U_{j \alpha}=\sqrt{\frac{2}{N+1}}
           \sin \left(\frac{j \alpha \pi}{N+1}\right)
        ...
        if not hasattr(self, "grids"):
            if self.doAnalytical:
                ...
                logger.warn(
                    "Sine DVR position operator is somehow "
                    + "different from straightforward integral."
                    + "See https://doi.org/10.1016/S0370-1573(99)00047-2 "
                    + "and Set doAnalytical=True"
                )
